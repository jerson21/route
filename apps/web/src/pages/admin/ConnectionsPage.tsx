      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Usuarios con sesion iniciada</p>
          </div>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="divide-y divide-gray-100 max-h-[500px] overflow-y-auto">
            {loading && users.length === 0 ? (
              ...
              <div className="p-8 text-center text-gray-500">
                ...
                <p className="text-sm mt-1">Los usuarios aparecen cuando inician sesion</p>
              </div>
            ) : (
              users.map(user => (
                <div
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200">
          ...
          <div className="p-4 space-y-4">
            {/* Selected User */}
            {selectedUser ? (
              <div className={`p-3 rounded-lg flex items-center gap-3 ${selectedUser.hasFcmToken ? 'bg-blue-50' : 'bg-yellow-50'}`}>
                <Smartphone className={`w-5 h-5 ${selectedUser.hasFcmToken ? 'text-blue-600' : 'text-yellow-600'}`} />
                <div className="flex-1">
                  <p className={`font-medium ${selectedUser.hasFcmToken ? 'text-blue-900' : 'text-yellow-900'}`}>{selectedUser.name}</p>
                  <p className={`text-sm ${selectedUser.hasFcmToken ? 'text-blue-700' : 'text-yellow-700'}`}>{selectedUser.email}</p>
                </div>
                {!selectedUser.hasFcmToken && (
                  <span className="text-xs bg-yellow-200 text-yellow-800 px-2 py-1 rounded">Sin FCM</span>
                )}
              </div>
            ) : (
            ...
            <button
              ...
              disabled={!selectedUser || !selectedUser.hasFcmToken || !messageBody.trim() || sending}
            ...
            {selectedUser && !selectedUser.hasFcmToken && (
              <p className="text-xs text-yellow-600 text-center">
                Este usuario no tiene FCM token registrado. Debe abrir la app Android para recibir notificaciones.
              </p>
            )}
          </div>
          ...
          <div className="p-4 border-t border-gray-200 bg-gray-50 rounded-b-xl">
            <p className="text-xs text-gray-500">
              <strong>Nota:</strong> Los mensajes se envian via Firebase Cloud Messaging (FCM).
              El icono verde indica que el usuario puede recibir notificaciones push.
            </p>
          </div>
        </div>
      </div>
